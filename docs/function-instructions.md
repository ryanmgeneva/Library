...
let result = books.filter((book) => book.borrows.some((borrower) => {return(borrower.id === account.id && borrower.returned === false)}))
result.forEach(books.map((book) => mumboo jumbo))

loop through books with for each, if book includes id from account id, send book to new array
loop through books in new array with for each, use find maybe on authors to get author for each maybe, return as new combined book author
...
book.borrows.map((borrow) => {
  let result = {...borrow, ...accounts.find((account) => { 
    account.id === (book.borrows.some((borrower) => borrower.id === account.id))
    }
  )}
})
